...
import { AppState, Experience, WeeklyPackage } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ appState, onSelectExperience, onGenerateNewWeek }) => {
  ...
  const weekExperiences = currentPackage.experiences.map(id => MOCK_EXPERIENCES.find(e => e.id === id)).filter(Boolean) as Experience[];
      ...
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
        <div>
           ...
           <p className="text-gray-500 mt-1">
             Your profile matches: <span className="font-medium text-primary">{userProfile.personality_data.traits.join(', ')}</span>
           </p>
        </div>
                ...
                <div className="grid md:grid-cols-2 gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                     <div className="order-2 md:order-1 flex flex-col justify-center space-y-4">
                         <div className="flex gap-2">
                             ...
                             <span className="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded font-medium">{todayExperience.estimated_time}</span>
                         </div>
                         <h3 className="text-3xl font-bold text-gray-900">{todayExperience.title}</h3>
                         <p className="text-gray-600 text-lg leading-relaxed">{todayExperience.description}</p>
                         <div className="pt-4">
                             <button 
                     ...
                     <div className="order-1 md:order-2 h-64 md:h-auto rounded-xl overflow-hidden relative group cursor-pointer" onClick={() => onSelectExperience(todayExperience.id)}>
                         <img src={todayExperience.imageUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" alt="Today" />
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {weekExperiences.map((exp, index) => {
             ...
             {/* Add a "Coming Soon" or "Surprise" card for fun */}
